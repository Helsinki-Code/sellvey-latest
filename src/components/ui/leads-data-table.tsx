...
export function LeadsTable({
      ...
      if (!snapshot.empty) {
        const loadedLeads = snapshot.docs.map(doc => {
          ...
          return {
            ...
            size: data.value || 0,
            interest: data.interest || [50, 50, 50, 50, 50],
            probability: data.probability || "mid",
            lastAction: data.updatedAt ? new Date(data.updatedAt).toLocaleDateString() : "Just now"
          } as Lead;
        });
        setLeads(loadedLeads);
      } else {
        // Seed some initial data if empty
        defaultLeads.forEach(async (lead) => {
          try {
            await addDoc(collection(db, "leads"), {
              name: lead.name,
              company: "Acme Corp",
              email: lead.email,
              status: lead.status === "pre-sale" ? "new" : lead.status === "closing" ? "proposal" : lead.status,
              value: lead.size,
  ...
  const filteredLeads = leads.filter(lead => {
    ...
    const probabilityMatch = probabilityFilter === "all" || lead.probability === probabilityFilter;
    return sourceMatch && statusMatch && probabilityMatch;
  });

  const uniqueSources = Array.from(new Set(leads.map(l => l.source)));
  const uniqueStatuses = Array.from(new Set(leads.map(l => l.status)));
  const uniqueProbabilities = Array.from(new Set(leads.map(l => l.probability)));
  ...
  const isAllSelected = selectedLeads.size === filteredLeads.length && filteredLeads.length > 0;
    ...
    <div className={`w-full max-w-7xl mx-auto ${className}`}>
      <div className="flex flex-wrap gap-4 mb-4">
        <div className="flex flex-col gap-1">
          <label className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground/60 ml-1">Source</label>
          <select 
            value={sourceFilter} 
            onChange={(e) => setSourceFilter(e.target.value)}
            className="bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-xs ethereal-text outline-none focus:ring-1 focus:ring-primary/50"
          >
        ...
        <div className="flex flex-col gap-1">
          <label className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground/60 ml-1">Status</label>
          <select 
            value={statusFilter} 
        ...
        <div className="flex flex-col gap-1">
          <label className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground/60 ml-1">Probability</label>
          <select 
          ...
          >
            ...
            {uniqueProbabilities.map(p => <option key={p} value={p}>{p.toUpperCase()}</option>)}
          </select>
        </div>
      </div>
      <div className="bg-transparent rounded-2xl overflow-hidden liquid-panel">
        <div className="grid grid-cols-7 gap-4 px-6 py-3 text-xs font-medium text-muted-foreground/70 uppercase tracking-wide bg-muted/5 border-b border-white/5">
              <div className="flex items-center gap-3">
                ...
                <span className="ethereal-text">Lead</span>
              </div>
              <div className="ethereal-text">Source</div>
              <div className="ethereal-text">Status</div>
              <div className="ethereal-text">Size</div>
              <div className="ethereal-text">Interest</div>
              <div className="ethereal-text">Probability</div>
              <div className="flex items-center gap-2 cursor-pointer ethereal-text" onClick={handleSort}>
                Last Action
        ...
        >
          {filteredLeads.map((lead, index) => (
            <motion.div key={lead.id} variants={rowVariants}>
              <div
                className={`grid grid-cols-7 gap-4 px-6 py-2 hover:bg-white/5 transition-colors cursor-pointer group relative ${
                  isSelected(lead.id) ? "bg-primary/10" : ""
              ...
              >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-8 h-8 bg-muted/10 rounded-full flex items-center justify-center flex-shrink-0 border border-white/10 liquid-panel">
                        <span className="text-sm font-medium ethereal-text">
                          {lead.name.charAt(0)}
                        </span>
                      </div>
                      <div className="min-w-0">
                        <div className="font-medium ethereal-text truncate">{lead.name}</div>
                        <div className="text-xs text-muted-foreground/70 ethereal-text truncate">{lead.email}</div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center">
                    <span className="font-semibold ethereal-text">
                    ...
                    <AnimatePresence mode="wait">
                      {hoveredAction === lead.id ? (
                        <motion.button
                          ...
                          className="flex items-center gap-2 px-2 py-1 bg-primary/10 hover:bg-primary/20 text-primary border border-primary/30 rounded-lg text-xs font-medium shadow-sm hover:shadow-md transition-all duration-200 cursor-pointer liquid-panel ethereal-text"
                        >
                          Engage
                        ...
                        <motion.span
                          ...
                          className="text-xs text-muted-foreground/70 ethereal-text"
                        >
                          {lead.lastAction}
                        </motion.span>
                      )}
